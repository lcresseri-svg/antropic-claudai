...
export function EditDefSheet({ open, draft, withKind, canDelete, onSave, onDelete, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-3"
      ...
      <div className="relative w-full max-w-md bg-elevated rounded-3xl p-6 shadow-float animate-sheet-up max-h-[90vh] overflow-y-auto scrollbar-hide">
        <div className="flex items-center gap-3 mb-5">
          ...
          <input value={label} onChange={e => setLabel(e.target.value)} placeholder="Nome" maxLength={24} autoFocus
            className="flex-1 bg-card rounded-2xl px-4 py-3 text-primary placeholder:text-secondary/50 outline-none focus:ring-1 focus:ring-gold/40" />
        </div>
          ...
          {EMOJI_CHOICES.map(e => (
            <button key={e} onClick={() => setIcon(e)}
              className={`aspect-square rounded-xl flex items-center justify-center text-lg transition-colors ${
                icon === e ? 'bg-gold/20 ring-1 ring-gold' : 'bg-card'
              }`}>{e}</button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {COLOR_CHOICES.map(c => (
            <button key={c} onClick={() => setColor(c)}
              className={`w-8 h-8 rounded-full transition-transform ${color === c ? 'ring-2 ring-offset-2 ring-offset-elevated ring-primary scale-110' : ''}`}
