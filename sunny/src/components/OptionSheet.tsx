...
export function OptionSheet({ open, title, options, onPick, onClose, footer }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-3"
      ...
      <div className="relative w-full max-w-md bg-elevated rounded-3xl shadow-float animate-sheet-up max-h-[72vh] flex flex-col border border-white/[0.04]">

        <div className="flex items-center justify-between px-6 pt-6 pb-4">
          <h3 className="text-base font-semibold text-primary">{title}</h3>
          <button onClick={onClose}
            className="w-8 h-8 rounded-full bg-card flex items-center justify-center text-secondary text-sm">✕</button>
        </div>

        <div className="grid grid-cols-2 gap-2 overflow-y-auto scrollbar-hide px-6 pb-6">
          {options.map(o => (
            <button key={o.id} onClick={() => onPick(o.id)}
              className="flex items-center gap-3 p-3.5 rounded-2xl bg-card active:bg-card-hover transition-colors text-left">
