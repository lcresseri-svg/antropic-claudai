 ...
 *
 * Un quadratino per giorno del mese corrente: quanto è pieno dice quanto hai
 * speso quel giorno. Tre stati possibili, mutuamente esclusivi:
 *
...
export function buildMonthRhythm(opts: {
  ...
  return {
    monthLabel: new Intl.DateTimeFormat('it-IT', { month: 'long' }).format(now),
    days,
    leadingBlanks,
    totalSpent,
    dailyAverage: elapsed.length > 0 ? r2(totalSpent / elapsed.length) : 0,
    todaySpent: days[todayDay - 1]?.spent ?? 0,
    peak,
